...
- **2026-05-29**: Completion roadmap locked (14 steps, dependency-ordered) via multi-agent workflow audit of current state vs CLAUDE.md. Critical path = card-gen → quality-engine → review-flow → stats. 10 spec decisions locked at defaults: 1 deck per mock test (`source=mock_test`, `sourceMockTestId`); model picks basic|cloze + `source_reference`=cluster theme (cloze in-scope v1); Quality Engine runs background via `after()` batching one Flash-Lite call per deck; quality rollup starts A, high −1 grade / medium −0.5 round down, **≥2 high warnings = needs_work**; retention metric = measured `(good+easy)/total` from review_logs (NOT stored retrievability); FSRS-5 defaults + retention 0.9 no fuzz; build shared `withGeminiFallback` (429→retry 5s→Claude Haiku); native in-app feedback form (uses `feedback` table); Anki import reads anki2+anki21b, export targets anki2 (universal), v1 drops media. Deferred: PII redaction before Gemini (Phase 2; beta relies on TOS disclosure — launch blocker for step 13).
- **2026-05-29**: DB query layer (Step 1) — `lib/db/queries/{decks,cards}.ts`. neon-http driver does NOT support interactive `db.transaction()` (verified: "No transactions support in neon-http driver"); use `db.batch([...])` (single server-side transaction, all-or-nothing). UUIDv7 ids generated JS-side so deck id is known before card inserts → createDeckWithCards = one batch (deck + cards + `generatedDeckId` backlink). `cardCount` denormalization kept in sync exclusively through these helpers (insert +1, delete `greatest(-1,0)`). All reads user-scoped via ownerId (no cross-user existence leak).
- **2026-05-20**: Phase 2 Session 7 — Gemini Vision analysis shipped. Model locked to `gemini-2.5-flash` (avoid 2.0 series sunset June 1, 3.1 preview instability per CLAUDE.md "AI providers"). Structured output via `generateObject` + Zod `AnalysisResultSchema` with 6 cluster types (vocabulary, grammar, collocation, reading_skill, writing_skill, task_response) and 3 severity levels (minor/moderate/major). Background pattern: `after()` from `next/server` triggered inside `recordUpload` server action — Next 16 native primitive, no `@vercel/functions` dep needed; runs in same function invocation after response is flushed. Cost tracked in `ai_cost_estimate_cents` (integer) from `response.usage.inputTokens/outputTokens` with `Math.ceil` to never under-report. Failure mode: `quality_warnings` JSONB column on `mock_tests` (new this session via migration `0002_friendly_mandroid.sql`) stores `analysis_failed` flag; detail page renders retry button that POSTs `/api/mock-tests/[id]/analyze` and clears the warning before re-running synchronously. Detection-time quality warnings (`low_image_quality`, `partial_answers`, `ambiguous_handwriting`, `language_mismatch`) emitted by Gemini when input is degraded — rendered as a yellow card above the cluster list. Distinct `MockTestQualityWarning` type from cards' `QualityWarning` (different rules). PostHog events: `mock_test_analyzed` (success, with band/cluster count/cost) and `mock_test_analysis_failed`. Known issue (deferred): `after()` doesn't reliably fire in `next dev` mode — first analysis post-upload requires manual retry on localhost. Production runtime unaffected; verified post-deploy. Known issue (deferred): handwriting/cursive OCR misreads possible (e.g. "tissues" → "threwt"); correction text is still right because Gemini reasons from context. Real student handwriting may exhibit similar misreads.
